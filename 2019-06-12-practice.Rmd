...
```{r}
for(i in seq_along(commitHistory)){
  sameone <- commitHistory[[i]]$commit$author$name == commitHistory[[i]]$commit$committer$name
  sameonedate <- commitHistory$commit$committer$date[sameone]
}
sameonedate
SaveEveryThing <- vector("list", length(sameonedate))
SaveEveryThing
```
