...
```{r ans3}
# nyTime
nyTime1 <- mdy_hms(submitTime)
nyTime1 <- with_tz(nyTime1, tz = "America/New_York")
nyTime <- as.character(nyTime1)
nyTime
```
...
```{r ans4}
# passData
grade60 <- randomGrades >= 60
passData <- list(
  成績=randomGrades[grade60],
  學分數=randomCredits[grade60]
)
passData
```
...
```{r ans52}
# westYears
westYears <- as.integer(str_sub(twDates2,1,3)) + 1911
...
```{r ans53}
# westDates
step1 <- str_split(twDates2,"年")
mm <- map(step1,~.[[2]])
westDates <- str_c(westYears,mm,01,sep="-")
...
```{r ans61}
# invoiceList
names(invoiceList)[[2]] <- c("訂單時區之下單時間") 
invoiceList
```
...
```{r ans71}
# failRate
a1 <- sampledTranscriptData$科目名稱 == "科目204"
a2 <- sampledTranscriptData$學期成績[a1]
failRate <- sum(a2 < 60)/length(a2)
failRate
```
...
```{r ans75}
# studentGPA
studentGPA1 <- sampledTranscriptData$學號=="410073409b" & sampledTranscriptData$學年==101 & sampledTranscriptData$學期==1
studentGPA2 <- sum(studentGPA1*sampledTranscriptData$學期成績*sampledTranscriptData$學分數)
studentGPA3 <- sum(studentGPA1*sampledTranscriptData$學分數)
studentGPA <- studentGPA2/studentGPA3
studentGPA
```
